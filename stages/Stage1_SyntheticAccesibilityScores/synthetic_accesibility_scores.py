...
def get_fingerprint_as_bit_counts(mol: rdkit.Mol) -> List[int]:
    ...
    """
    info = dict()  # :noqa
    # Ensure molecules has hydrogens added.
    mol = rdkit.AddHs(mol)
...
class SyntheticAccesibilityScorer:
    def __init__(
        ...
        sa_funcs="default",
        train=False,
        sa_model_path="utilities/RFModel_AddHs.joblib",
    ):
        if sa_funcs == "default":
...
def main():
    ...
    # model = Model()
    # data = model.load_data("Data/Training_SA_Data_AddHs.json")
    # model.train(data)
    # model.dump("utilities/RFModel_AddHs.joblib")
    # # Test example.
    mol = rdkit.MolFromSmiles(
        "OCc1ccc(C(=C(c2ccc(CO)cc2)c2ccc(CO)cc2)c2ccc(CO)cc2)cc1"
    )
